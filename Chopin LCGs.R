# ------------------------------------------------------------
# LCN / LCG generator (Chopin, Op. 28)
# - Means: -6..6
# - Reps: 50 at -6 and +6; 100 at -5..+5
# - SD: drawn a priori from 7..15 for EACH normal
# - Truncate by rejection to [-37, 40]
# - Sample EXACTLY n_per_normal accepted draws per normal
# - Round to nearest integer pitch, bin to -37..40
# - Output: 3-column table (index, pitch, incidence)
# ------------------------------------------------------------

make_chopin_lcn_table_big <- function(
    prelude_id = 1,
    base_seed = 1000,
    L = -37, U = 40,
    n_per_normal = 775   # 1200 * 775 = 930,000 total pitches
) {
  ...
  for (i in seq_along(means)) {
    ...
    for (j in seq_len(reps[i])) {
      ...
      # Chopin SD draw (a priori, symmetric)
      sd <- sample(7:15, 1)
...
# ------------------------
tab1 <- make_chopin_lcn_table_big(prelude_id = 1)

sum(tab1$incidence)   # should be exactly 930000
tab1

# optional save
write.csv(tab1, "Chopin_LCN_Prelude01_pitch_table.csv", row.names = FALSE)