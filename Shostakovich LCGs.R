# ------------------------------------------------------------
# LCN / LCG generator (Shostakovich, Op. 87)
# - Means: -6..6
# - Reps: 50 at -6 and +6; 100 at -5..+5
# - SD: drawn a priori from 9..17 for EACH normal
# - Truncate by rejection to [-40, 44]
# - Sample EXACTLY n_per_normal accepted draws per normal
# - Round to nearest integer pitch, bin to -40..44
# - Output: 3-column table (index, pitch, incidence)
# ------------------------------------------------------------

make_shostakovich_lcn_table_big <- function(
    prelude_id = 1,
    base_seed = 1000,
    L = -40, U = 44,
    n_per_normal = 715   # 1200 * 715 = 858,000 total pitches
) {
...
# ------------------------
tab1 <- make_shostakovich_lcn_table_big(prelude_id = 1)

sum(tab1$incidence)   # should be exactly 858000
tab1

# optional save
write.csv(tab1, "Shostakovich_LCN_Prelude01_pitch_table.csv", row.names = FALSE)