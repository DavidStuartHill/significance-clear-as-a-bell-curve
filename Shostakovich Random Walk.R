# New Shostakovich -1,0,1 (with boundary-hit counts)
...
# --- Parameters for Shostakovich ---
steps <- 714

start_positions <- c(-5:-1, 0, 1:5, -6, 6)

# Updated boundaries
reset_position_low  <- -40
reset_position_high <- 44

num_runs <- ifelse(abs(start_positions) == 6, 50, 100)

# --- Run ---
result_shostakovich <- random_walk_count_multiple_starts(
  steps, start_positions, reset_position_low, reset_position_high, num_runs
)

# Histogram
walk_counts_df_aggregated <- as.data.frame(result_shostakovich$position_counts)
colnames(walk_counts_df_aggregated) <- c("Position", "Count")
print(walk_counts_df_aggregated)

# Boundary-hit counts
print(result_shostakovich$boundary_hits)